...
class $modify(ltLevelInfoLayer, LevelInfoLayer) {
    ...
    void loadCustomLevelInfoLayer() {
        if (!Mod::get()->getSettingValue<bool>("levelInfoShow") || !m_level) return;
...
void updateTags() {
    auto contain = this->getChildByID("level-tags");
    if (!contain) return;
    ...
    void showNoTagsMessage() {
        if (Mod::get()->getSettingValue<bool>("HideNoTags")) {
            this->removeChildByID("level-tags");
            this->getChildByID("title-label")->setPositionY(303);
            return;
        }
        auto containerMenu = this->getChildByID("level-tags");
